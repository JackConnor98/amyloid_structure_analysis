...
# Cleaning Environment
rm(list = c("CoM_data", "e2e_data", "Rg_data", "mean_data", "i"))
...
# Selecting only chain A from df 
constant_df <- df %>% filter(chain == "A")
...
for (i in 1:nrow(min_df)) {
  ...
  if (min_df$chain[i] == "A") {
    ...
    min_df$pdb_id[i] <- min_df$pdb[i]
    
  } else {
    
    min_df$pdb_id[i] <- paste0(min_df$pdb[i], "_", min_df$chain[i])
